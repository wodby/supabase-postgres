...
def prepare():
    ...
    if os.environ.get('PGDATA', str(DATA)) != str(DATA):
        raise ValueError('The Supabase image requires PGDATA=/var/lib/postgresql/data')
    if os.environ.get('POSTGRES_USER', 'supabase_admin') != 'supabase_admin':
        raise ValueError('The Supabase image requires POSTGRES_USER=supabase_admin')
    if os.environ.get('POSTGRES_DB', 'postgres') != 'postgres':
        raise ValueError('The Supabase image requires POSTGRES_DB=postgres')
